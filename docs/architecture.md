...
Architecture distinguishes workspace packages from external dependencies with a scope filter and legend. `graph`/`projects` accept `includeVendors: true` to additionally inspect `package.json` and `Cargo.toml` under `vendor`/`vendors` directories, including Git-ignored vendor folders. This opt-in scan skips symlinks, hidden folders, archives and build/cache directories, caps discovery at 30,000 entries and 1,000 manifests, and reads at most 1 MB per manifest. Vendor scripts are never executed. Returned vendor dependencies are manifest declarations, not inferred installed resolution; target-specific Cargo sections and inherited workspace dependencies are not resolved by this scanner. Results are added to the response without contaminating the ordinary graph cache.

`component_metadata` collects technical details on demand for 1–20 raw `memberIds` sharing a component directory. It is available to desktop, CLI and MCP. It returns collection time, first commit visible for the current directory (not rename-following creation history), last modifying commit (SHA, committer date, author, subject), latest inventoried file mtime, dirty status, and file size/count. Size covers regular tracked/unignored files, including nested projects but excluding build/cache trees and symlinks, capped at 20,000 files. Unreadable/deleted files and shallow history produce warnings. Ignored vendor files and remote packages have no claimed installed size.

The desktop requests metadata automatically when a component is selected. Results are cached by repository, raw member identities and analysis options for 30 seconds; Refresh details forces a new collection for that component. Failed requests remain visible and can be retried manually. Nx affected remains optional and requires repository trust.

Integrity records distinguish measured SHA-256 manifest fingerprints from declared pnpm lockfile SRI and exact Cargo.lock crate checksums. Lockfile declarations do not verify downloaded archives. Missing or ambiguous checksums stay unavailable. Nx affected requires `checkAffected: true`, trusted execution, an Nx member identity, and explicit `base`/`head` references (defaults `main`/`HEAD`); Git resolves them to immutable commits before `nx show projects --affected --json`. This comparison excludes uncommitted changes. Untrusted, failed or inapplicable Nx queries return `affected: null` with a reason, never a misleading false.
