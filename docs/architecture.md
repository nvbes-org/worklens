...
The passive JavaScript catalogue lists tracked/unignored manifests, including archived directories if they contain manifests. It is not a claim that every manifest is an active pnpm workspace member. The trusted pnpm connector adds installed resolution evidence (depth 1); full recursive package resolution is not claimed. Cargo resolution uses default features and all target configurations; offline failure falls back to workspace-member metadata with partial status and no resolved edges/features.
