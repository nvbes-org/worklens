...
- `prUrls`: at most one canonical GitHub PR URL, explicitly selected from this work item's confirmed links. This opts into authenticated GitHub reads for a [PR evidence dossier](work-evidence.md). Candidate, rejected, unlinked and malformed URLs are rejected before network access. No remote evidence is selected by default.
...
- Without `prUrls`, export makes no GitHub requests or keychain reads. The `links` section still exports PR/issue references only. Discussions, CI logs, hidden conversations, unselected notes and full history are never included.
